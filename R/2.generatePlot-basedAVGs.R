...
dataFlexAutoSummary <- summaryBy(time + unionSize + minSize + resultSize ~ model + m + variables+relations, data = dataFlexAutoSummary, FUN = function(x) { c(mean = mean(x)) } )

dataFlexAutoSummary$accuracy<-dataFlexAutoSummary$unionSize/dataFlexAutoSummary$minSize
dataFlexAutoSummary$minimality <- dataFlexAutoSummary$minSize / dataFlexAutoSummary$resultSize

dataFlexAutoSummary <- subset(dataFlexAutoSummary, select = -c(unionSize.mean,minSize.mean) )
dataFlexAutoSummary<-dataFlexAutoSummary[c("model","m","variables","relations","time.mean","resultSize.mean","minimality","accuracy")]
...
dataFlexGenSummary <- summaryBy(time + unionSize + minSize + resultSize~ features + m, data = dataFlexGen, FUN = function(x) { c(mean = mean(x)) } )

dataFlexGenSummary$accuracy.mean<-dataFlexGenSummary$unionSize/dataFlexGenSummary$minSize
dataFlexGenSummary$minimality.mean<- dataFlexGenSummary$minSize / dataFlexGenSummary$resultSize

dataFlexGenSummary <- subset(dataFlexGenSummary, select = -c(unionSize.mean,minSize.mean) )
dataFlexGenSummary<-dataFlexGenSummary[c("features","m","time.mean","resultSize.mean","minimality.mean","accuracy.mean")]
...
######## Evolutionary 
dataEvolSummary <- summaryBy(unionSize+resultSize+minSize+time  ~ features + m, data = dataEvol, FUN = function(x) { c(mean = mean(x)) } )

dataEvolSummary$accuracy.mean<-dataEvolSummary$unionSize/dataEvolSummary$minSize
dataEvolSummary$minimality.mean<- dataEvolSummary$minSize / dataEvolSummary$resultSize

dataEvolSummary <- subset(dataEvolSummary, select = -c(unionSize.mean,minSize.mean) )
dataEvolSummary<-dataEvolSummary[c("features","m","time.mean","resultSize.mean","minimality.mean","accuracy.mean")]
...
##prepare summary
mergedDataSummary <- summaryBy(unionSize+resultSize+minSize+time  ~ features + m, data = mergedData, FUN = function(x) { c(mean = mean(x)) } )

mergedDataSummary$accuracy.mean<-mergedDataSummary$unionSize/mergedDataSummary$minSize
mergedDataSummary$minimality.mean<- mergedDataSummary$minSize / mergedDataSummary$resultSize

mergedDataSummary <- subset(mergedDataSummary, select = -c(unionSize.mean,minSize.mean) )
mergedDataSummary<-mergedDataSummary[c("features","m","time.mean","resultSize.mean","minimality.mean","accuracy.mean")]
...
dataFlexRealTotalSummary <- summaryBy(unionSize+resultSize+minSize+time  ~ features + m + model, data = dataFlexReal, FUN = function(x) { c(mean = mean(x)) } )


dataFlexRealTotalSummary$accuracy.mean<-dataFlexRealTotalSummary$unionSize/dataFlexRealTotalSummary$minSize
dataFlexRealTotalSummary$minimality.mean<- dataFlexRealTotalSummary$minSize / dataFlexRealTotalSummary$resultSize

dataFlexRealTotalSummary <- subset(dataFlexRealTotalSummary, select = -c(unionSize.mean,minSize.mean) )
dataFlexRealTotalSummary<-dataFlexRealTotalSummary[c("model","features","m","time.mean","resultSize.mean","minimality.mean","accuracy.mean")]
...
## Only Debian data.
dataFlexRealDebianSummary<- summaryBy(unionSize+resultSize+minSize+time  ~ features + m + model, data = dataFlexReal[which(dataFlexReal$model =="xenial.xml"),], FUN = function(x) { c(mean = mean(x)) } )
dataFlexRealDebianSummary$accuracy.mean<-dataFlexRealDebianSummary$unionSize/dataFlexRealDebianSummary$minSize
dataFlexRealDebianSummary$minimality.mean<- dataFlexRealDebianSummary$minSize / dataFlexRealDebianSummary$resultSize

dataFlexRealDebianSummary <- subset(dataFlexRealDebianSummary, select = -c(unionSize.mean,minSize.mean) )
dataFlexRealDebianSummary<-dataFlexRealDebianSummary[c("model","features","m","time.mean","resultSize.mean","minimality.mean","accuracy.mean")]
...
## Without Debian data.
dataFlexRealNoDebianSummary<- summaryBy(unionSize+resultSize+minSize+time  ~ features + m + model, data = dataFlexReal[which(!dataFlexReal$model =="xenial.xml"),], FUN = function(x) { c(mean = mean(x)) } )
dataFlexRealNoDebianSummary$accuracy.mean<-dataFlexRealNoDebianSummary$unionSize/dataFlexRealNoDebianSummary$minSize
dataFlexRealNoDebianSummary$minimality.mean<- dataFlexRealNoDebianSummary$minSize / dataFlexRealNoDebianSummary$resultSize

dataFlexRealNoDebianSummary <- subset(dataFlexRealNoDebianSummary, select = -c(unionSize.mean,minSize.mean) )
dataFlexRealNoDebianSummary<-dataFlexRealNoDebianSummary[c("model","features","m","time.mean","resultSize.mean","minimality.mean","accuracy.mean")]
